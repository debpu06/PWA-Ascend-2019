...
namespace EPiServer.Reference.Commerce.B2B.CustomerProcessors
{
    ...
    public class B2BPlacedPriceProcessor : DefaultPlacedPriceProcessor
    {
        ...
        public override bool UpdatePlacedPrice(ILineItem lineItem, CustomerContact customerContact, MarketId marketId, Currency currency, Action<ILineItem, ValidationIssue> onValidationError)
        {
            ...
            if (lineItem.Properties["SubscriptionPrice"] != null && !string.IsNullOrEmpty(lineItem.Properties["SubscriptionPrice"].ToString()))
            {
                if (lineItem.PlacedPrice != Convert.ToDecimal(lineItem.Properties["SubscriptionPrice"].ToString()))
                {
                    lineItem.PlacedPrice = Convert.ToDecimal(lineItem.Properties["SubscriptionPrice"].ToString());
                }
                return true;
            }

            Money? placedPrice = GetPlacedPrice(entryContent, lineItem.Quantity, customerContact, marketId, currency);
