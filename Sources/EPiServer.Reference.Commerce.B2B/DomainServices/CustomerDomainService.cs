...
namespace EPiServer.Reference.Commerce.B2B.DomainServices
{
    ...
    public class CustomerDomainService : ICustomerDomainService
    {
        ...
        public IEnumerable<B2BContact> GetContacts()
            => CustomerContext.Current.GetContacts(0, 1000).Select(c => new B2BContact(c));

        public CustomerContact GetCustomerByEmail(string email)
        {
            return CustomerContext.Current.GetContacts(0, 1000).FirstOrDefault(user => user.Email == email);
        }
    }
}
