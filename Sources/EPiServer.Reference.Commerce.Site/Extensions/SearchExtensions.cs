...
namespace EPiServer.Reference.Commerce.Site.Extensions
{
    public static class SearchExtensions
    {
        ...
        private static FilterBuilder<T> GetOrFilterForStringList<T>(IEnumerable<string> fieldValues,
            ...
            string fieldName)
        {
            var filters = fieldValues.Select(s => new TermFilter(fieldName, s)).
                Cast<Filter>().
                ToList();
        ...
        public static ITypeSearch<T> AddFilterForNumericRange<T>(this ITypeSearch<T> query,
            ...
            Type type)
        {
            return query.Filter(GetOrFilterForNumericRange(query, range, fieldName, type));
        }
        ...
        public static ITypeSearch<T> AddFilterForIntList<T>(this ITypeSearch<T> query,
            ...
            string fieldName)
        {
            return categories.Any() ? query.Filter(GetOrFilterForIntList(query, categories, fieldName, null)) : query;
        }
            ...
            Type type)
        {
            ...
            var filters = values.Select(value => new TermFilter(fullFieldName, value)).
                Cast<Filter>().
                ToList();
