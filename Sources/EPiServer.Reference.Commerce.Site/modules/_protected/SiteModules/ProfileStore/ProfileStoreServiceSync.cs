...
{
    public class ProfileStoreServiceSync
    {
        ...
        public ProfileStoreServiceSync(
        ...
        public ProfileStoreServiceSync()
            : this(
                  ServiceLocator.Current.GetInstance<CountryManagerFacade>(),
                  ServiceLocator.Current.GetInstance<ITrackingService>(),
        ...
        public ProfileStoreItems GetAllProfiles(ProfileStoreFilterOptions profileStoreFilterOptions)
        {
            ...
            {
                response = Request(HttpMethod.Get, $"{apiBaseUrl}/{profilesUrl}/");
            }
            else
            {
                response = Request(HttpMethod.Get, $"{apiBaseUrl}/{profilesUrl}/{CreateFilterOptionsUrl(profileStoreFilterOptions)}");
            }

            string getProfiles = response.Content.ReadAsStringAsync().Result;
        ...
        public ProfileStoreItems GetProfiles(string queryString)
        {
            HttpResponseMessage response;
            var profileStoreItems = new ProfileStoreItems();

            string uri = $"{apiBaseUrl}/{profilesUrl}/{queryString}";
            response = Request(HttpMethod.Get, uri);
            var getProfileStoreItems = response.Content.ReadAsStringAsync().Result;
        ...
        public ProfileStoreModel GetProfileById(string scope, Guid profileId)
        {
            var profileStoreModel = new ProfileStoreModel();
            var response = Request(HttpMethod.Get, $"{apiBaseUrl}/{profilesUrl}/{scope}/{profileId.ToString()}");
            string getProfile = response.Content.ReadAsStringAsync().Result;
        ...
        public void EditOrCreateProfile(string scope, ProfileStoreModel model)
        {
            var profileId = model.ProfileId == null ? Guid.NewGuid().ToString() : model.ProfileId.ToString();
            Request(HttpMethod.Put, $"{apiBaseUrl}/{profilesUrl}/{profileId}", JsonConvert.SerializeObject(model));
        }
        ...
        public ScopeItems GetAllScopes()
        {
            var response = Request(HttpMethod.Get, $"{apiBaseUrl}/{scopesUrl}");
            var getScopes = response.Content.ReadAsStringAsync().Result;
        ...
        public ScopeItems GetScopesById(Guid scopeId)
        {
            var response = Request(HttpMethod.Get, $"{apiBaseUrl}/{scopesUrl}/{scopeId.ToString()}");
            var getScope = response.Content.ReadAsStringAsync().Result;
            var scopeItems = new ScopeItems();
        ...
        public SegmentItems GetAllSegments()
        {
            var response = Request(HttpMethod.Get, $"{apiBaseUrl}/{segmentsUrl}/?$top=10000&$filter=Archived eq false");
            var getSegments = response.Content.ReadAsStringAsync().Result;
        ...
        public SegmentItems GetSegmentById(Guid segmentId)
        {
            var response = Request(HttpMethod.Get, $"{apiBaseUrl}/{segmentsUrl}/{segmentId}");
            var getSegment = response.Content.ReadAsStringAsync().Result;
            var segmentItems = new SegmentItems();
        ...
        public SegmentItems GetSegments(string queryString)
        {
            HttpResponseMessage response;
            var segmentItems = new SegmentItems();

            string uri = $"{apiBaseUrl}/{segmentsUrl}{queryString}";
            response = Request(HttpMethod.Get, uri);
            var getSegmentItems = response.Content.ReadAsStringAsync().Result;

            if (string.IsNullOrEmpty(getSegmentItems))
            {
                segmentItems = new SegmentItems()
                {
                    SegmentList = new List<SegmentModel>(),
            ...
            {
                segmentItems = JsonConvert.DeserializeObject<SegmentItems>(getSegmentItems);
            }

            return segmentItems;
        }

        public void EditOrCreateSegment(SegmentModel model)
        {
            var segmentId = model.SegmentId == null ? Guid.NewGuid().ToString() : model.SegmentId.ToString();
            Request(HttpMethod.Put, $"{apiBaseUrl}/{segmentsUrl}/{segmentId}", JsonConvert.SerializeObject(model));
        }
        #endregion

        #region Blacklist Api

        public BlacklistItems GetAllBlacklist()
        {
            var response = Request(HttpMethod.Get, $"{apiBaseUrl}/{blacklistsUrl}");
            var getBlacklists = response.Content.ReadAsStringAsync().Result;
        ...
        public BlacklistItems GetBlacklistById(Guid blaclistId)
        {
            var response = Request(HttpMethod.Get, $"{apiBaseUrl}/{blacklistsUrl}/{blaclistId}");
            var getBlacklist = response.Content.ReadAsStringAsync().Result;
            var blacklistItems = new BlacklistItems();
        ...
        public VisualizationItems GetVisualizationItems(string queryString)
        {
            HttpResponseMessage response;
            var visualizationItems = new VisualizationItems();

            string uri = $"{apiBaseUrl}/{trackEventsUrl}/{queryString}";
            response = Request(HttpMethod.Get, uri);
            var getVisualizationItems = response.Content.ReadAsStringAsync().Result;
        ...
        public TrackEventItems GetAllTrackEvents(ProfileStoreFilterOptions profileStoreFilterOptions)
        {
            HttpResponseMessage response;
            ...
            if (profileStoreFilterOptions == null)
            {
                response = Request(HttpMethod.Get, $"{apiBaseUrl}/{trackEventsUrl}/");
            }
            else
            {
                string uri = $"{apiBaseUrl}/{trackEventsUrl}/{CreateFilterOptionsUrl(profileStoreFilterOptions)}";
                response = Request(HttpMethod.Get, uri);
            }

            var getTrackEvents = response.Content.ReadAsStringAsync().Result;
        ...
        public void LoadCountry(ProfileStoreModel profileModel)
        {
            profileModel.CountryOptions = GetAllCountries();
        }
        ...
        {
            var allCountries = _countryManager.GetCountries().Country.Select(x => new CountryViewModel { Code = x.Code, Name = x.Name }).ToList();
