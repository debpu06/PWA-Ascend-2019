...
namespace EPiServer.Reference.Commerce.Site.Features.InsightToPostClick
{
    ...
    public class PostClickInterceptor : ITrackingDataInterceptor
    {
        ...
        {
            var connectorId = ConnectForCampaign.Core.Helpers.Constants.ConnectorId.ToString();
            var instanceId = ConnectForCampaign.Core.Helpers.Constants.DefaultConnectorInstanceId.ToString();

            if (string.IsNullOrEmpty(connectorId) || string.IsNullOrEmpty(instanceId))
            {
                return null;
            }
        ...
        {
            UrlBuilder urlBuilder = new UrlBuilder(_postClickBaseUrl);
