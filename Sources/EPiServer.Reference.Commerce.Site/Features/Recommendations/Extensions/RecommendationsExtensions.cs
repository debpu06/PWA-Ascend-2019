...
namespace EPiServer.Reference.Commerce.Site.Features.Recommendations.Extensions
{
    public static class RecommendationsExtensions
    {
        ...
        public static IEnumerable<Recommendation> GetCartRecommendations(this TrackingResponseData response, ReferenceConverter referenceConverter)
        {
            return response.GetRecommendationGroups(referenceConverter).Where(x => x.Area == Basket).SelectMany(x => x.Recommendations);
        }

        public static IEnumerable<Recommendation> GetRecommendations(this TrackingResponseData response, ReferenceConverter referenceConverter, string area)
        {
            return response.GetRecommendationGroups(referenceConverter).Where(x => x.Area == area).SelectMany(x => x.Recommendations);
        }
    }
}