...
namespace EPiServer.Reference.Commerce.Site.Features.Cart.ViewModelFactories
{
    ...
    public class CartItemViewModelFactory
    {
        ...
        public virtual CartItemViewModel CreateCartItemViewModel(ICart cart, ILineItem lineItem, EntryContentBase entry)
        {
            ...
            var variant = entry as VariantBase;
            if (variant != null)
            {
        ...
        private IEnumerable<string> GetAvailableSizes(BaseProduct product, VariantBase entry)
        {
            return product != null && entry != null ?
                _productService.GetVariants(product).OfType<VariantBase>().Where(x => string.IsNullOrEmpty(x.Color) || string.IsNullOrEmpty(entry.Color) || x.Color.Equals(entry.Color))
                .Select(x => x.Size)
