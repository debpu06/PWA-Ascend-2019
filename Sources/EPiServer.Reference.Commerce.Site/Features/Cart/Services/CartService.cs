...
namespace EPiServer.Reference.Commerce.Site.Features.Cart.Services
{
    ...
    public class CartService : ICartService
    {
        ...
        public string DefaultCartName => "Default" + SiteDefinition.Current.StartPage.ID.ToString();

        public string DefaultWishListName => "WishList" + SiteDefinition.Current.StartPage.ID.ToString();

        public string DefaultSharedCardName => "Shared" + SiteDefinition.Current.StartPage.ID.ToString();
        ...
        public AddToCartResult AddToCart(ICart cart, string code, decimal quantity, string deliveryMethod, string warehouseCode)
        {
            ...
            if (entryContent is BundleContent)
            {
                foreach (var relation in _relationRepository.GetChildren<BundleEntry>(contentLink))
                {
                    ...
                    var recursiveResult = AddToCart(cart, entry.Code, relation.Quantity ?? 1, deliveryMethod, warehouseCode);
        ...
        {
            return LoadCart(name, _customerContext.CurrentContactId.ToString(), validate);
        }

        public CartWithValidationIssues LoadCart(string name, string contactId, bool validate)
        {
            var validationIssues = new Dictionary<ILineItem, List<ValidationIssue>>();
            var cart = !string.IsNullOrEmpty(contactId) ? _orderRepository.LoadCart<ICart>(new Guid(contactId), name, _currentMarket) : null;
            if (cart != null)
            {
        ...
        {
            return LoadOrCreateCart(name, _customerContext.CurrentContactId.ToString());
        }
        ...
        private Dictionary<ILineItem, List<ValidationIssue>> RemoveLineItem(ICart cart, int shipmentId, string code)
        {
            if (cart.GetFirstForm().Shipments.Any())
            {
                ...
                if(cart.GetFirstForm().GetAllLineItems().Count() == 0)
                {
        ...
        {
            var cart = _orderRepository.LoadCart<ICart>(currentContactId, DefaultSharedCardName, _currentMarket);
            if (cart == null)
            {
                return cart;
            }
