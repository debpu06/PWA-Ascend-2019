...
namespace EPiServer.Reference.Commerce.Site.Features.Product.ViewModelFactories
{
    ...
    public class CatalogEntryViewModelFactory
    {
            ...
            where TViewModel : ProductViewModelBase<TProduct, TVariant>, new()
        {
            ...
            if (!TryGetVariant(variants, variationCode, out TVariant variant))
            {
                return new TViewModel
                {
                    ...
                    Reviews = new ReviewsViewModel(),
                    Variants = new List<VariantViewModel>(),
                    ColorSwatches = new List<SwatchViewModel>(),
                    SizeSwatches = new List<SwatchViewModel>(),
                    UseImagesForVariantsSelection = (currentContent as BaseProduct).UseImagesForVariantsSelection
                };
            }
            ...
            var baseVariant = variant as VariantBase;
            ...
            return new TViewModel
            {
                ...
                Colors = variants.OfType<VariantBase>()
                ...
                Sizes = variants.OfType<VariantBase>()
                ...
                Variants = variants.Select(x =>
                {
                    ...
                    {
                        Sku = x.Code,
                        Size = x is VariantBase ? $"{(x as VariantBase).Color} {(x as VariantBase).Size}" : "",
                ...
                ColorSwatches = variants.OfType<VariantBase>()
                    .Where(x => x.Color != null)
                    .Select(x =>
                    {
                        var variantImage = x.GetAssets<IContentImage>(_contentLoader, _urlResolver, SwatchImageGroup.Color.ToString()).FirstOrDefault();
                        return new SwatchViewModel
                        {
                            Selected = false,
                            Value = x.Color,
                            Text = x.Color,
                            ImageUrl = string.IsNullOrEmpty(variantImage) ? "http://placehold.it/150x150?text=" + x.Color.Replace(" ","+") : variantImage,
                        };
                    }).GroupBy(o => o.Value).Select(g => g.OrderBy(s => s.ImageUrl).First()).ToList(),
                SizeSwatches = variants.OfType<VariantBase>()
                    .Where(x => (x.Color == null || x.Color.Equals(baseVariant?.Color, StringComparison.OrdinalIgnoreCase)) && x.Size != null)
                    .Select(x =>
                    {
                        var variantImage = x.GetAssets<IContentImage>(_contentLoader, _urlResolver, SwatchImageGroup.Size.ToString()).FirstOrDefault();
                        return new SwatchViewModel
                        {
                            Selected = false,
                            Value = x.Size,
                            Text = x.Size,
                            ImageUrl = string.IsNullOrEmpty(variantImage) ? "http://placehold.it/150x150?text="+ x.Size.Replace(" ", "+") : variantImage,
                        };
                    }).GroupBy(o => o.Value).Select(g => g.OrderBy(s => s.ImageUrl).First()).ToList(),
                UseImagesForVariantsSelection = (currentContent as BaseProduct).UseImagesForVariantsSelection
            };
        ...
        public virtual VariantBase SelectVariant(BaseProduct currentContent, string color, string size)
        {
            var variants = GetVariants<VariantBase, BaseProduct>(currentContent);

            if (TryGetFashionVariantByColorAndSize(variants, color, size, out VariantBase variant)
        ...
        private static bool TryGetFashionVariantByColorAndSize(IEnumerable<VariantBase> variants, string color, string size, out VariantBase variant)
        {
