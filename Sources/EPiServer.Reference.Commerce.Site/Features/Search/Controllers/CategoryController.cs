...
namespace EPiServer.Reference.Commerce.Site.Features.Search.Controllers
{
    public class CategoryController : ContentController<FashionNode>
    {
        ...
        public async Task<ViewResult> Index(FashionNode currentContent, FilterOptionViewModel viewModel)
        {
            ...
            var orderedProducts = model.ProductViewModels.ToList().OrderByDescending(x => Convert.ToInt32(browsedItems.Any(y => y.Code.Equals(x.Code))));
            model.ProductViewModels = orderedProducts;
        ...
        [ChildActionOnly]
        public ActionResult RecentlyBrowsed(FashionNode currentContent)
        {
