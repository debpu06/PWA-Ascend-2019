...
namespace EPiServer.Reference.Commerce.Site.Features.Destinations
{
    public class TagsPartialRouting : IPartialRouter<TagPage, TagPage>
    {
        ...
        public object RoutePartial(TagPage content, Web.Routing.Segments.SegmentContext segmentContext)
        {
            ...
            if (!string.IsNullOrEmpty(continentPart.Next))
            {
                ...
                var mcount = SearchClient.Instance.Search<DestinationPage>().Filter(dp => dp.TagString().Match(content.Name)).Filter(dp => dp.Continent.MatchCaseInsensitive(continent)).Take(0).GetContentResult().TotalMatching;
