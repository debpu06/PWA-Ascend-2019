...
namespace EPiServer.Reference.Commerce.Site.Features.Social.Repositories.ActivityStreams
{
    ...
    public class PageSubscriptionRepository : IPageSubscriptionRepository
    {
        ...
        {
            return new Subscription(EPiServer.Social.Common.Reference.Create(subscription.Subscriber),
                EPiServer.Social.Common.Reference.Create(subscription.Target));
        }
        ...
        private SubscriptionFilter AdaptSubscriptionFilter(PageSubscriptionFilter filter)
        {
            ...
            {
                Subscriber = !string.IsNullOrWhiteSpace(filter.Subscriber) ? EPiServer.Social.Common.Reference.Create(filter.Subscriber) : EPiServer.Social.Common.Reference.Empty,
                Target = !string.IsNullOrWhiteSpace(filter.Target) ? EPiServer.Social.Common.Reference.Create(filter.Target) : EPiServer.Social.Common.Reference.Empty,
            };
        }
    }
}