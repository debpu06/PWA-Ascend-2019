...
namespace EPiServer.Reference.Commerce.Site.Infrastructure.Jobs
{
    ...
    public class DemoIntegrationJob : ScheduledJobBase
    {
        ...
        private List<string> GetCodes()
        {
            ...
            {
                connection.Open();
                var command = new SqlCommand("Select code from catalogentry where ClassTypeId in ('Variation', 'Package')", connection)
                {
        ...
        private void UpdateCatalogContentRecursive(ContentReference parentLink, CultureInfo defaultCulture)
        {
            foreach (var child in LoadChildrenBatched(parentLink, defaultCulture))
            {
                ...
                _contentRepository.Save(child, DataAccess.SaveAction.Publish, Security.AccessLevel.NoAccess);
