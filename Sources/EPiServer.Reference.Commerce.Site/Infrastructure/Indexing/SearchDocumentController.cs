...
namespace EPiServer.Reference.Commerce.Site.Infrastructure.Indexing
{
    ...
    public class SearchDocumentController : ApiController
    {
        ...
        protected RestSearchDocument PopulateRestSearchDocument(string code, string language)
        {
            ...
            var fashionProduct = entryContent as BaseProduct;
            var fashionPackage = entryContent as PackageContent;
            if (fashionProduct != null)
            {
                var variants = _contentLoader.GetItems(fashionProduct.GetVariants(_relationRepository), CultureInfo.GetCultureInfo(language)).OfType<VariantBase>().ToList();
        ...
        private void AddSizes(RestSearchDocument document, IEnumerable<VariantBase> variants)
        {
        ...
        private void AddColors(RestSearchDocument document, IEnumerable<VariantBase> variants)
        {
        ...
        private void AddCodes(RestSearchDocument document, IEnumerable<VariantBase> variants)
        {
