...
namespace EPiServer.Reference.Commerce.Site.Infrastructure.Indexing
{
    public class CatalogIndexer : CatalogIndexBuilder
    {
        ...
        private IEnumerable<VariantBase> GetVariants(IEnumerable<BaseProduct> products, CultureInfo language)
        {
        ...
        private IEnumerable<BaseProduct> GetProducts(EntryContentBase entryContent, CultureInfo language)
        {
            switch (entryContent.ClassTypeId)
            {
                case EntryType.Package:
                    return _contentLoader.GetItems(((PackageContent)entryContent).GetEntries(), language).OfType<BaseProduct>();

                case EntryType.Bundle:
                    return _contentLoader.GetItems(((BundleContent)entryContent).GetEntries(), language).OfType<BaseProduct>();

                case EntryType.Product:
                    return new[] { entryContent as BaseProduct };
            }

            return Enumerable.Empty<BaseProduct>();
        }
        ...
        private void AddSizes(ISearchDocument document, IEnumerable<VariantBase> variants)
        {
        ...
        private void AddColors(ISearchDocument document, IEnumerable<VariantBase> variants)
        {
        ...
        private void AddCodes(ISearchDocument document, IEnumerable<VariantBase> variants)
        {
        ...
        private void AddBrands(ISearchDocument document, IEnumerable<BaseProduct> products)
        {
